...
if [ "$MAIN_NAME" = "main" ]
then
  ficherosEntrada=("script_minimos/new" "script_minimos/bid" "script_minimos/delete" "script_minimos/award" "script_minimos/withdraw" "script_minimos/remove" "script_minimos/invalidate")
  ficherosRef=("script_minimos/new_ref" "script_minimos/bid_ref" "script_minimos/delete_ref" "script_minimos/award_ref" "script_minimos/withdraw_ref" "script_minimos/remove_ref" "script_minimos/invalidate_ref")
else
  ficherosEntrada=("")
  ficherosRef=("script_test/test_product_list_ref" "script_test/test_bid_stack_ref")
fi
...
if [ "$MAIN_NAME" = "main" ]
then
  check_output "./main" 2 "bid_stack.c product_list.c" ${VERBOSE}
  MAIN_OK=${allOK}
...
#8 de abril, primer seguimiento
  ...
  STACK_OK=${allOK}
  show_result "Bid stack test:" ${STACK_OK}
  
  printf "\n"
  
  ficherosRef=("script_test/test_product_list_ref")
  check_output "./test_product_list" 0 "product_list.c" ${VERBOSE}
  LIST_OK=${allOK}
  show_result "Product list test:" ${LIST_OK}
  ...
  show_result "Bid stack test:" ${STACK_OK}
  show_result "Product list test:" ${LIST_OK}
  show_result "Tests global result:" $((${STACK_OK} * ${LIST_OK}))
